...
week.results <- full_join(results,detections) %>% 
  ...
  mutate(week = week(filedate)) %>% 
  
  group_by(station,deployment,sp_code,week) %>% 
  summarise(presence = sum(presence),
            detection = sum(detection)) %>% 
  ungroup()

fig.results <- week.results %>% 
  group_by(station,deployment,sp_code) %>% 
  summarise(weeks_detect = sum(presence >0 & detection > 0),
            weeks_nodetect = sum(presence>0 & detection ==0)) %>% 
  mutate(percent = (weeks_detect/(weeks_detect+weeks_nodetect))*100) %>% 
  
  
  mutate(year = str_extract(deployment, "\\d{4}")) %>% 
  
  pivot_longer(cols = c(weeks_detect, weeks_nodetect),
               names_to = "detection",
               values_to = "weeks") %>% 
  mutate(detection = recode(as.character(detection),
                            weeks_detect = "Presence, Detected by Species Detector",
                            weeks_nodetect = "Presence, Not Detected by Species Detector")) %>% 
...
for (i in years) {
  ...
  p <-ggplot()+
    ...
    geom_bar_pattern(data = year.fig.results, aes(x = station, y = weeks, fill = sp_code, pattern = detection, pattern_fill=sp_code), 
    ...
    geom_text(data = year.fig.results %>% group_by(station,sp_code,percent) %>% summarise(weeks=sum(weeks)) %>% ungroup(), aes(x = station, y= weeks+2, label = paste0(round(percent), "%"))) +
    
    labs(title = paste0(i," Deployments: ", sp_label[unique(year.fig.results$sp_code)]," - weekly"),
         x = "Station",
         y= "Weeks with Presence") +
  ...
  plotname <- paste0(i,"_",single_sp,"_singlevmulti_weekly.png")
  
  ggsave(paste0(r"(C:\Users\mackling\Documents\Baleen PAM\Single_v_Multispecies\)",plotname),p, width = 9.5, height =  4.5, units = "in" )
}
