...
<<<<<<< HEAD
detections <- read_csv(paste0(r"(R:\Science\CetaceanOPPNoise\CetaceanOPPNoise_5\BaleenWhale_AcousticAnalysis\Deployments\)",params$deployment,
                              r"(\Validation\Arklite_Inputs\)",paste0(params$deployment,'_MBDetections.csv')), show_col_types = FALSE)%>% 
...
=======
detections <- read_csv(paste0(r"(R:\Science\CetaceanOPPNoise\CetaceanOPPNoise_5\BaleenWhale_AcousticAnalysis\Deployments\)",deployment,
                              r"(\Validation\Arklite_Inputs\)",paste0(deployment,'_MBDetections.csv')), show_col_types = FALSE)%>% 
  mutate(Species= case_when(Species=="Bm"~"BmT", TRUE~Species))
...
annotations <- read_csv(paste0(r"(R:\Science\CetaceanOPPNoise\CetaceanOPPNoise_5\BaleenWhale_AcousticAnalysis\Deployments\)",deployment,
                               r"(\Results\)", deployment,"_DPA_MB_Annotations_FINAL.csv", sep=""))
>>>>>>> c113ce9cf2872065c9c39e46099d8011b3842493
...
=======
  { if (Bm_audible == FALSE) select(.,any_of(detec_list)) %>%  mutate(.,"BWA-2"=NA_real_, "BWA-4"=NA_real_, .after="BWT-4")
>>>>>>> c113ce9cf2872065c9c39e46099d8011b3842493
...
for (i in rownames(TableFileLFDCS)) {
  ...
  if(!is.na(Filedata2[[i]])) {
    TableFileLFDCS[i,"Precision"]<-  precision(Filedata2,get(str_extract(i,'[a-zA-Z]+')),get(i))[3]
  }
  else {TableFileLFDCS[i,"Precision"]<- NA
  }
}
...
for (i in rownames(TableFileLFDCS)) {
  ...
  if(!is.na(Filedata2[[i]])) {
    TableFileLFDCS[i,"Recall"]<-  recall(Filedata2,get(str_extract(i,'[a-zA-Z]+')),get(i))[3]
  }
  else {TableFileLFDCS[i,"Recall"]<- NA
  }
}
...
for (i in rownames(TableFileLFDCS)) {
  ...
  if(!is.na(Filedata2[[i]])) {
    TableFileLFDCS[i,"MCC"]<- mcc(Filedata2,get(str_extract(i,'[a-zA-Z]+')),get(i))[3]
  }
  else {TableFileLFDCS[i,"MCC"]<- NA
  }
}
...
for (i in rownames(TableDayLFDCS)) {
  ...
  if(!is.na(Daydata2[[i]])) {
    TableDayLFDCS[i,"Precision"]<-  precision(Daydata2,get(str_extract(i,'[a-zA-Z]+')),get(i))[3]
  }
...
for (i in rownames(TableDayLFDCS)) {
  ...
  if(!is.na(Daydata2[[i]])) {
    TableDayLFDCS[i,"Recall"]<-  recall(Daydata2,get(str_extract(i,'[a-zA-Z]+')),get(i))[3]
  }
...
for (i in rownames(TableDayLFDCS)) {
  ...
  if(!is.na(Daydata2[[i]])) {
    TableDayLFDCS[i,"MCC"]<- mcc(Daydata2,get(str_extract(i,'[a-zA-Z]+')),get(i))[3]
  }
